...
export default function TeamPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.error(
        editing ? "Failed to update team member" : "Failed to add team member",
      );
    } finally {
      setSaving(false);
    }
  ...
  const togglePublished = async (member: TeamMember) => {
    try {
      await teamService.update(member._id, {
        isPublished: !member.isPublished,
      });
      fetchData();
    } catch {
    ...
    <div>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          />
          <div
            className="relative bg-white rounded-xl shadow-2xl w-full max-w-lg mx-4 p-6 max-h-[85vh] overflow-y-auto"
        ...
        <DataTable
          columns={[
            ...
            },
            {
              key: "role",
              header: "Role",
              render: (m) => <span className="text-slate-600">{m.role}</span>,
            },
            {
              key: "order",
              header: "Order",
              render: (m) => <span className="text-slate-400">{m.order}</span>,
            },
            {
              key: "status",
              header: "Status",
              render: (m) => (
                ...
                >
                  <StatusBadge
                    status={m.isPublished ? "verified" : "unverified"}
                  />
                </button>
              ),
            },
